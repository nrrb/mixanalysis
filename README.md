...
On the Upload tab, when you upload two or more mixes you can label each file as a **goal-level mix** (a reference you want to sound more like) or an **aspiring mix** (one of your own mixes). When exactly one goal mix is labeled, the Compare Mixes tab adds plain-English suggestions for how each aspiring mix could change to flow more like the goal.
